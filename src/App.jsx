import { useState, useEffect } from "react";

const languages = ["EN", "RU"];

import bgImage from "./assets/images/bg3.png";

export default function App() {
  const [lang, setLang] = useState("EN");
  const [langOpen, setLangOpen] = useState(false);
  const [menuOpen, setMenuOpen] = useState(false);

  useEffect(() => {
    const closeAll = (e) => {
      if (!e.target.closest("#lang-selector")) {
        setLangOpen(false);
      }
      if (
        !e.target.closest("#mobile-menu") &&
        !e.target.closest("#burger-button")
      ) {
        setMenuOpen(false);
      }
    };
    window.addEventListener("click", closeAll);
    return () => window.removeEventListener("click", closeAll);
  }, []);

  return (
    <div className="relative w-full h-full min-h-screen">
      {/* Фоновое изображение */}
      <div
        className="absolute inset-0 bg-cover bg-top bg-no-repeat z-0"
        style={{ backgroundImage: `url(${bgImage})` }}
      />

      {/* Всё остальное поверх фона */}
      <div className="relative z-10">
        {/* Хедер */}
        <header className="flex items-center justify-between px-6 py-4 bg-opacity-50 text-white select-none">
          {/* Левое меню */}
          <nav className="hidden md:flex space-x-8 font-semibold text-md uppercase text-white">
            <a href="#" className="hover:text-indigo-400 transition-colors">
              Home
            </a>
            <a href="#" className="hover:text-indigo-400 transition-colors">
              E-bikes
            </a>
            <a href="#" className="hover:text-indigo-400 transition-colors">
              Connect
            </a>
            <a href="#" className="hover:text-indigo-400 transition-colors">
              About
            </a>
          </nav>
          {/* Центр — логотип */}
          <h1 className="font-[900] text-[32px] md:text-5xl flex items-center leading-[38px] tracking-wide text-white">
            GIZMO
          </h1>
          {/* Правый блок */}
          <div className="hidden md:flex items-center space-x-8 font-semibold text-md">
            <a href="#" className="hover:text-indigo-400 transition-colors">
              Test Ride
            </a>

            {/* Языковой селектор */}
            <div id="lang-selector" className="relative">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setLangOpen((v) => !v);
                }}
                className="flex items-center space-x-1 hover:text-indigo-400 focus:outline-none transition-colors"
                type="button"
              >
                <span>{lang}</span>
                <svg
                  className={`w-4 h-4 transition-transform ${
                    langOpen ? "rotate-180" : "rotate-0"
                  }`}
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  viewBox="0 0 24 24"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M19 9l-7 7-7-7"
                  ></path>
                </svg>
              </button>
              <ul
                className={`absolute right-0 mt-2 bg-black bg-opacity-80 rounded shadow-lg w-20 text-center z-40 transition-transform origin-top ${
                  langOpen
                    ? "scale-100 opacity-100"
                    : "scale-95 opacity-0 pointer-events-none"
                }`}
                style={{ transition: "all 0.15s ease-in-out" }}
              >
                {languages.map((l) => (
                  <li
                    key={l}
                    className={`py-1 cursor-pointer hover:bg-indigo-600 ${
                      l === lang ? "font-bold" : ""
                    }`}
                    onClick={() => {
                      setLang(l);
                      setLangOpen(false);
                    }}
                  >
                    {l}
                  </li>
                ))}
              </ul>
            </div>

            {/* Корзина */}
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-6 w-6 cursor-pointer hover:text-indigo-400 transition-colors"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                d="M22.6453 6.06938C22.5404 5.93132 22.405 5.81938 22.2497 5.7423C22.0944 5.66521 21.9234 5.62507 21.75 5.625H5.64656L4.83375 2.69906C4.76798 2.46215 4.6264 2.2533 4.43068 2.10448C4.23496 1.95566 3.99587 1.87506 3.75 1.875H1.875C1.57663 1.875 1.29048 1.99353 1.0795 2.2045C0.868526 2.41548 0.75 2.70163 0.75 3C0.75 3.29837 0.868526 3.58452 1.0795 3.7955C1.29048 4.00647 1.57663 4.125 1.875 4.125H2.895L6.11062 15.7022C6.26546 16.2547 6.59651 16.7415 7.05341 17.0886C7.51032 17.4357 8.06809 17.624 8.64187 17.625H17.9062C18.48 17.624 19.0378 17.4357 19.4947 17.0886C19.9516 16.7415 20.2827 16.2547 20.4375 15.7022L22.8403 7.05094C22.8858 6.88337 22.8918 6.70757 22.858 6.53728C22.8241 6.36699 22.7514 6.20684 22.6453 6.06938ZM18.2625 15.1003C18.2408 15.1785 18.1944 15.2475 18.1301 15.297C18.0659 15.3465 17.9873 15.3739 17.9062 15.375H8.64C8.55809 15.3749 8.47847 15.348 8.41329 15.2984C8.34812 15.2488 8.30097 15.1792 8.27906 15.1003L6.27187 7.875H20.2697L18.2625 15.1003ZM10.125 20.625C10.125 20.9958 10.015 21.3584 9.809 21.6667C9.60298 21.975 9.31014 22.2154 8.96753 22.3573C8.62492 22.4992 8.24792 22.5363 7.8842 22.464C7.52049 22.3916 7.1864 22.213 6.92417 21.9508C6.66195 21.6886 6.48337 21.3545 6.41103 20.9908C6.33868 20.6271 6.37581 20.2501 6.51773 19.9075C6.65964 19.5649 6.89996 19.272 7.20831 19.066C7.51665 18.86 7.87916 18.75 8.25 18.75C8.74728 18.75 9.22419 18.9475 9.57582 19.2992C9.92745 19.6508 10.125 20.1277 10.125 20.625ZM19.875 20.625C19.875 20.9958 19.765 21.3584 19.559 21.6667C19.353 21.975 19.0601 22.2154 18.7175 22.3573C18.3749 22.4992 17.9979 22.5363 17.6342 22.464C17.2705 22.3916 16.9364 22.213 16.6742 21.9508C16.412 21.6886 16.2334 21.3545 16.161 20.9908C16.0887 20.6271 16.1258 20.2501 16.2677 19.9075C16.4096 19.5649 16.65 19.272 16.9583 19.066C17.2666 18.86 17.6292 18.75 18 18.75C18.4973 18.75 18.9742 18.9475 19.3258 19.2992C19.6775 19.6508 19.875 20.1277 19.875 20.625Z"
                fill="white"
              />
            </svg>
          </div>
          {/* Мобильный гамбургер */}
          <button
            id="burger-button"
            aria-label="Toggle menu"
            onClick={(e) => {
              e.stopPropagation();
              setMenuOpen((v) => !v);
            }}
            className="md:hidden space-y-1.5 focus:outline-none ml-2"
          >
            <span
              className={`block w-6 h-0.5 bg-white transition-transform origin-left ${
                menuOpen ? "rotate-45 translate-y-1.5" : ""
              }`}
            />
            <span
              className={`block w-6 h-0.5 bg-white transition-opacity ${
                menuOpen ? "opacity-0" : "opacity-100"
              }`}
            />
            <span
              className={`block w-6 h-0.5 bg-white transition-transform origin-left ${
                menuOpen ? "-rotate-45 -translate-y-1.5" : ""
              }`}
            />
          </button>
        </header>

        {/* Мобильное меню слева */}
        <div
          id="mobile-menu"
          className={`fixed top-0 left-0 h-full w-64 bg-black bg-opacity-90 text-white z-50 transform transition-transform duration-300 ease-in-out
          ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}
        >
          <nav className="flex flex-col justify-center h-full space-y-8 text-center text-xl font-semibold select-none">
            <a
              href="#"
              className="hover:text-indigo-400 transition-colors"
              onClick={() => setMenuOpen(false)}
            >
              Home
            </a>
            <a
              href="#"
              className="hover:text-indigo-400 transition-colors"
              onClick={() => setMenuOpen(false)}
            >
              E-Bikes
            </a>
            <a
              href="#"
              className="hover:text-indigo-400 transition-colors"
              onClick={() => setMenuOpen(false)}
            >
              Connect
            </a>
            <a
              href="#"
              className="hover:text-indigo-400 transition-colors"
              onClick={() => setMenuOpen(false)}
            >
              About
            </a>
          </nav>
        </div>

        {/* Затемнение фона при открытом меню */}
        {menuOpen && (
          <div
            className="fixed inset-0 bg-black bg-opacity-50 z-40"
            onClick={() => setMenuOpen(false)}
          />
        )}

        {/* Основной контент страницы */}
        <main className="flex-grow"></main>
      </div>
    </div>
  );
}
